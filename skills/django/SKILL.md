...
### Migrations: one per pull request, squashed before review, frozen once merged

**Principle.** A pull request that changes models arrives with **exactly one**
migration, and once that migration is merged it is never edited again. Several
migration files while a branch is in progress is normal; shipping them is not,
because each one is a separate schema change reviewers have to read and production
has to survive in order.

The rule has one reason behind it, and it is worth stating plainly: **a merged
migration has already run somewhere.** The deploy applies migrations on every
release, so by the time a change reaches main the schema it describes exists in a
real database. Editing that file afterwards changes only what a *fresh* database
gets. Every environment that already migrated keeps the old shape, silently, and
the two drift apart until something fails far from the edit.

While the branch is open, squash instead:

```bash
rm <app>/migrations/00XX_*.py        # the ones this branch added, and only those
python manage.py makemigrations <app> -n <name>
python manage.py makemigrations --check --dry-run   # must print "No changes detected"
```

Then prove it on a database that has never seen the branch, because that is the
only way the dependency chain and the reversibility get exercised:

```bash
createdb <project>_migrationcheck && DATABASE_URL=...<project>_migrationcheck \
  python manage.py migrate
```

What the squash does not survive without care:

- **Data migrations.** A `RunPython` step written by hand is not regenerated by
  `makemigrations`. Copy it into the new file before deleting the old ones, and
  keep its `reverse_code` (or `migrations.RunPython.noop`, deliberately).
- **A migration that merged while you were working.** Your regenerated file may
  claim a number that now exists on main. Rebase, delete yours, regenerate: never
  renumber by hand, and never edit the one that merged to make room.
- **Squashing is not `squashmigrations`.** Django's command is for compacting
  *released* history across many migrations; this is simply regenerating a branch's
  own unreleased file. Do not reach for it here.

When a merged migration turns out to be wrong, the fix is **a new migration** that
corrects it, reviewed like any other change.

**This project** (`profile.json` → `migrations`). `one_per_pr` says whether the rule
is enforced, `squash_command` and `verify_commands` give the exact steps, and
`applied_by` names what runs `migrate` on deploy, which is what makes a merged
migration frozen. `pr_environments` is the trap worth reading before assuming an
open branch is harmless: an environment that deploys pull requests applies their
migrations to a shared database, so a branch's throwaway migration can land in
production history. Details, including how to repair that, are in
...
- `references/migrations.md` — the squash recipe step by step, data migrations
  through a squash, colliding numbers, repairing a migration that reached
  production from a pull request environment, and what to do when a merged
  migration is wrong.
